...
export default function EditPostPage() {
  ...
  const DRAFT_KEY = `blog_post_draft_${id}`
  ...
  }, [loadingPost, id, DRAFT_KEY, setValue])
  ...
  const fetchPost = async () => {
    try {
      ...
      // 如果文章已有标签，选中它们
      if (data.tags) {
        setSelectedTagIds(data.tags.map((t: { id: number }) => t.id))
      }
    } catch (err) {
      console.error(err)
      error('加载失败')
  ...
  const generateUniqueSlug = async (baseSlug: string, currentId: number): Promise<string> => {
    ...
    const data = await res.json()
    // 排除当前编辑的文章，只检查其他文章是否有相同slug
    const existingSlugs = data.posts.filter((p: any) => p.id !== currentId).map((p: any) => p.slug)
  ...
  }, [title, slug, excerpt, content, published, selectedTagIds, DRAFT_KEY])

  // 防抖自动保存
  useEffect(() => {
    if (!loadingPost && (title || content)) {
      const timer = setTimeout(() => {
        saveDraft()
      }, 1000)
      return () => clearTimeout(timer)
    }
  }, [title, slug, excerpt, content, published, loadingPost, saveDraft])

  const onSubmit = async (data: PostFormData) => {
    setLoading(true)
    try {
      // 检查slug是否和其他文章重复，如果重复自动生成唯一slug
