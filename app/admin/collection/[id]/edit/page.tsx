...
export default function EditCollectionPage({ params }: { params: { id: string } }) {
  ...
  const DRAFT_KEY = `blog_collection_draft_${params.id}`
  ...
  }, [loadingData, DRAFT_KEY, setValue])
  ...
  const fetchCollection = async () => {
    try {
      ...
      if (data.collection) {
        ...
        setValue('name', data.collection.name)
        setValue('slug', data.collection.slug)
        setValue('description', data.collection.description || '')
        setValue('cover', data.collection.cover || '')
        setValue('published', data.collection.published)
      }
    } catch (err) {
  ...
  }, [name, slug, description, cover, published, DRAFT_KEY])

  // 防抖自动保存
  useEffect(() => {
    if (!loadingData && (name || description)) {
      const timer = setTimeout(() => {
        saveDraft()
      }, 1000)
      return () => clearTimeout(timer)
    }
  }, [name, slug, description, cover, published, loadingData, saveDraft])
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">编辑专栏</h1>
        {(hasDraft || autoSaving) && (
